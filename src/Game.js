...
// const Game = ({ mode, logPaddleMovement }) => { // ✅ Added mode and logPaddleMovement props
//     const { totalTransactions, latency, updateTransactions } = useTracking();
//     const canvasRef = useRef(null);
//     const ctxRef = useRef(null);
//     const livesRef = useRef(3);
...
//     const { paddleX, updatePaddlePosition } = useControls(CANVAS_WIDTH, PADDLE_WIDTH, logPaddleMovement);
...
//     const logTransactionAndPause = async () => {
...
//             let newBallSpeedY = checkBrickCollision(ballX.current, ballY.current, ballSpeedY.current, bricks, levelConfig.width, levelConfig.height, updateTransactions);
//             if (newBallSpeedY !== ballSpeedY.current) await logTransactionAndPause();
//             ballSpeedY.current = newBallSpeedY;

//             const wallCollision = checkWallCollision(ballX.current, ballY.current, BALL_RADIUS, ballSpeedX.current, ballSpeedY.current, CANVAS_WIDTH, CANVAS_HEIGHT, updateTransactions);
//             if (wallCollision.newBallSpeedX !== ballSpeedX.current || wallCollision.newBallSpeedY !== ballSpeedY.current) await logTransactionAndPause();
//             ballSpeedX.current = wallCollision.newBallSpeedX;
//             ballSpeedY.current = wallCollision.newBallSpeedY;

//             const paddleCollision = checkPaddleCollision(ballX.current, ballY.current, BALL_RADIUS, ballSpeedY.current, paddleX, PADDLE_WIDTH, CANVAS_HEIGHT, updateTransactions);
//             if (paddleCollision.newBallSpeedY !== ballSpeedY.current) await logTransactionAndPause();
//             ballSpeedY.current = paddleCollision.newBallSpeedY;

//             if (paddleCollision.lostLife) {
//                 await logTransactionAndPause();
//                 loseLife(livesRef, setGameOver, () => {
//                     resetBallAndPaddle(ballX, ballY, ballSpeedX, ballSpeedY, paddleX);
//                 });
//             }
...
//     return (
//         <>
//             <canvas ref={canvasRef} width={CANVAS_WIDTH} height={CANVAS_HEIGHT} style={{ background: "#eee", display: "block", margin: "auto" }} />
//             <GameStats totalTransactions={totalTransactions} latency={latency} lives={livesRef.current} />
//             <LatencyGraph latency={latency} />
//             {gameOver && <h2 style={{ textAlign: "center" }}>Game Over!</h2>}
//         </>
...
//             drawBricks(ctx, bricks, levelConfig.width, levelConfig.height, levelConfig.padding, levelConfig.offsetLeft, levelConfig.offsetTop, levelConfig.brickColor);
//             drawBall(ctx, ballX.current, ballY.current, BALL_RADIUS, BALL_COLOR);
//             drawPaddle(ctx, paddleX.current, CANVAS_HEIGHT - PADDLE_HEIGHT, PADDLE_WIDTH, PADDLE_HEIGHT, PADDLE_COLOR);

//             // Handle Brick Collision
//             ballSpeedY.current = checkBrickCollision(
//                 ballX.current, ballY.current, ballSpeedY.current, bricks,
//                 levelConfig.width, levelConfig.height, updateTransactions
//             );
